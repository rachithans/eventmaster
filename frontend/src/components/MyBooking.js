...
export default function MyBooking({ userId }) {
  ...
  const handleCancellation = async (eventId, price) => {
    ...
    try {
      ...
      setModalBody(["Your tickets are cancelled. Have a nice day.", `Return amount ($${price*checkCount[eventId]}) = Qty(${checkCount[eventId]}) X Price($${price})`])
    } catch (error) {
  ...
  const handleModification = async (eventId, qty, price) => {
    ...
    if(count[eventId] === checkCount[eventId]){
      ...
      setModalBody(["Please user '-' to decrease the number of tickets or '+' to increase the number of tickets."])
    }
    else if(qty>0){
    const data = {
      userId: userId,
    ...
    try {
      ...
      setModalBody(["Your ticket are modified. Enjoy your  event.", `New Total Price ($${price*qty}) = Qty(${qty}) X Price($${price})`])
      setTimeout(200);
      getRecords();
    } catch (error) {
    ...
    setModalBody(["To modify the tickets, you should have atleast 1 ticket.", ``])
  }
  };
    ...
    <div className="container my-4">
      ...
      {records.map((record) => (
        <div className="card" key={record.eventId}>
          <div className="card-body">
            ...
            <div className="d-flex justify-content-between align-items-center">
              ...
              <button className="normal-button" onClick={() =>
                  handleModification(record.eventId, count[record.eventId], record.ticketPrice)
                  
                }data-bs-toggle="modal" data-bs-target="#modifyButton">
                Modify Booking
              </button>

              {/* https://getbootstrap.com/docs/5.3/components/modal/ */}
              <div className="modal fade" id="modifyButton" tabindex="-1" aria-labelledby="modifyButtonLabel" aria-hidden="true">
                <div className="modal-dialog">
                  <div className="modal-content">
                    <div className="modal-header">
                      <h1 className="modal-title fs-5" id="modifyButtonLabel">{modalTitle}</h1>
                    </div>
              ...
              <button
                ...
                onClick={() => handleCancellation(record.eventId, record.ticketPrice)}
                data-bs-toggle="modal" data-bs-target="#cancelButton"
              >
                Cancel Booking
              </button>
              ...
              <div className="modal fade" id="cancelButton" tabindex="-1" aria-labelledby="cancelButtonLabel" aria-hidden="true">
                <div className="modal-dialog">
                  <div className="modal-content">
                    <div className="modal-header">
                      <h1 className="modal-title fs-5" id="cancelButtonLabel">{modalTitle}</h1>
                    </div>
                    <div className="modal-body">
                      <p>{modalBody[0]}</p>
                      <p>{modalBody[1]}</p>
          ...
          <p><b className="price">Total Payment: ${checkCount[record.eventId]*record.ticketPrice}</b></p>
        </div>
        </div>
      ))}
    </div>
